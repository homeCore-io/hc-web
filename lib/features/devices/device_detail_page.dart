...
class _DeviceDetailView extends ConsumerWidget {
        ...
        children: [
          ...
          if (device.state.isEmpty)
            const Padding(
              ...
              child: Text('No controllable attributes',
  ...
  Future<void> _showEditDialog(
    ...
    final allDevices =
        ref.read(devicesProvider).valueOrNull ?? [];
  ...
  Future<void> _send(dynamic newValue) async {
    ...
    try {
      await ref.read(devicesApiProvider).setDeviceState(
          widget.deviceId, {widget.attribute: newValue});
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  }
